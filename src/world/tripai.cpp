  ...
  for (l = 0; l < 4; l++) {
    ...
    case MEKAN_MISSION_PUSH_IN:
      ...
      case MEKAN_PUSH_IN_MOVE_TO_PLANE:
        ...
        } else if (mekan_direction[l] == MEKAN_DIRECTION_LEFT &&
                   (mekan_x[l] + 13) <= (player_x_8[mekan_target[l]] - 10)) {
    ...
    case MEKAN_MISSION_PUSH_OUT:
      ...
      if (mekan_subtarget[l] &&
          (player_speed[mekan_target[l]] || !plane_present[mekan_target[l]])) {
      ...
      case MEKAN_PUSH_OUT_PUSH:
        ...
        if (mekan_direction[l] == MEKAN_DIRECTION_RIGHT) {
          ...
          if (mekan_x[l] <=
              (leveldata.airfield_x[l] + PLANE_AIRFIELD_POSITION + 5)) {
            plane_wants_out[mekan_target[l]] = 0;
            mekan_mission[l] = MEKAN_MISSION_RETURN;
...
int check_structs(int x, int y, int number) {
  ...
  for (l = 0; l < MAX_AA_GUNS; l++)
    if (kkbase_x[l] && player_sides[kkbase_country[l]] == number &&
        (kkbase_status[l] != 2) && (bombs_going[kkbase_number[l]] == -1)) {
      if (x >= kkbase_x[l] && x <= (kkbase_x[l] + 25) &&
          y >= (kkbase_y[l] - 5) && y <= (kkbase_y[l] + 30)) {
  ...
  for (l = 0; l < MAX_STRUCTURES; l++)
    if (leveldata.struct_owner[l] != 4)
      if ((structures[l][1] != nullptr) && (!struct_state[l]) &&
          (player_sides[leveldata.struct_owner[l]] == number) &&
          (bombs_going[l] == -1)) {
        if (x >= leveldata.struct_x[l] &&
            x <= (leveldata.struct_x[l] + struct_width[l]) &&
            y >= (leveldata.struct_y[l] - 5) &&
            y <= (leveldata.struct_y[l] + 5 + struct_heigth[l])) {
          bomb_target = l;
          return 1;
        }
      }
...
int check_multi_structs(int x, int y, int number) {
  ...
  for (l = 0; l < MAX_AA_GUNS; l++)
    if (kkbase_x[l] && kkbase_country[l] != 4 &&
        player_sides[kkbase_country[l]] != player_sides[number] &&
        (kkbase_status[l] != 2) && (bombs_going[kkbase_number[l]] == -1)) {
      if (x >= kkbase_x[l] && x <= (kkbase_x[l] + 25) &&
          y >= (kkbase_y[l] - 5) && y <= (kkbase_y[l] + 30)) {
        bomb_target = kkbase_number[l];
  ...
  for (l = 0; l < MAX_STRUCTURES; l++)
    if (leveldata.struct_owner[l] != 4)
      if ((structures[l][1] != nullptr) && (!struct_state[l]) &&
          (player_sides[leveldata.struct_owner[l]] != player_sides[number]) &&
          (bombs_going[l] == -1)) {
        if (x >= leveldata.struct_x[l] &&
            x <= (leveldata.struct_x[l] + struct_width[l]) &&
            y >= (leveldata.struct_y[l] - 5) &&
...
void ai_do_bombing(int number) {
  ...
  while (1) {
    ...
    if (fake_y >= 0)
      if (terrain_level[fake_x >> 8] < (fake_y >> 8) ||
          ((fake_y >> 8) >= 200)) {
        if (fake_y >= 614400)
          return;
...
void do_it_shots() {
  ...
  for (l = 0; l < MAX_ITGUN_SHOTS; l++) {
    if (itgun_shot_x[l]) {
      ...
      if ((itgun_shot_x[l] < 0) ||
          ((itgun_shot_x[l] >> 8) >= NUMBER_OF_SCENES * 160) ||
          (itgun_shot_y[l] >> 8) >= 200 || itgun_shot_y[l] < 0)
        itgun_shot_x[l] = 0;
      else if ((level_bitmap[(itgun_shot_x[l] >> 8) +
                             (itgun_shot_y[l] >> 8) * 2400]) < 112 ||
               ((level_bitmap[(itgun_shot_x[l] >> 8) +
...
void infan_to_plane(int l) {
  ...
  for (l2 = 0; l2 < 16; l2++) {
    ...
    if ((player_on_airfield[infan_country[l]] ||
         ((abs(player_x_8[l2] - player_x_8[infan_country[l]]) > 150) ||
          !plane_present[infan_country[l]])) &&
        angle <= 180) {
      infan_state[l] = 1;
...
void infan_to_struct(int l) {
  ...
  for (l2 = 0; l2 < MAX_AA_GUNS; l2++)
    if (kkbase_x[l2] && kkbase_status[l2] != 2 &&
        (leveldata.struct_owner[kkbase_number[l2]] != 4) &&
        (player_sides[leveldata.struct_owner[kkbase_number[l2]]] !=
         player_sides[infan_country[l]])) {
  ...
  for (l2 = 0; l2 < MAX_STRUCTURES; l2++)
    if ((structures[l2][1] != nullptr) && (leveldata.struct_owner[l2] != 4) &&
        (player_sides[leveldata.struct_owner[l2]] !=
         player_sides[infan_country[l]]) &&
        (!struct_state[l2])) {
      if (infan_x[l] + 15 >= leveldata.struct_x[l2] &&
          infan_x[l] <= (leveldata.struct_x[l2] + struct_width[l2])) {
        if (config.flames)
          start_flame(leveldata.struct_x[l2],
                      leveldata.struct_y[l2] + struct_heigth[l2],
                      struct_width[l2]);
        ...
        if (current_mode == SVGA_MODE) {
          ...
          structures[l2][1]->blit_to_bitmap(
              standard_background,
              leveldata.struct_x[l2] -
                  (leveldata.struct_x[l2] / screen_width) * screen_width,
              leveldata.struct_y[l2] +
                  (leveldata.struct_x[l2] / screen_width) * 196 - 4);

          structures[l2][1]->blit(
              leveldata.struct_x[l2] -
                  (leveldata.struct_x[l2] / screen_width) * screen_width,
              leveldata.struct_y[l2] +
                  (leveldata.struct_x[l2] / screen_width) * 196 - 4);
          ...
          if ((leveldata.struct_x[l2] -
            ...
            structures[l2][1]->blit(
                leveldata.struct_x[l2] -
                    ...
                    screen_width,
                leveldata.struct_y[l2] +
                    (leveldata.struct_x[l2] / screen_width + 1) * 196 - 4);
        }

        if (leveldata.struct_hit[l2]) {
          structures[l2][1]->blit_to_bitmap(maisema, leveldata.struct_x[l2],
                                            leveldata.struct_y[l2]);
        }
      }
    }
...
void infan_take_hits(int l) {
  ...
  for (l2 = util::wutil::wrandom(3); l2 < MAX_SHOTS; l2 += 3) {
    ...
    if ((shots_flying_x[l2] >> 8) > infan_x[l] &&
        (shots_flying_x[l2] >> 8) < (infan_x[l] + 14) &&
        (shots_flying_y[l2] >> 8) > infan_y[l] &&
        (shots_flying_y[l2] >> 8) < (infan_x[l] + 13) &&
        shots_flying_infan[l2] != infan_country[l]) {
      // if(wrandom(3))
      //   continue;
        ...
        if (abs(kkbase_x[l] - player_x_8[l2]) < 150) {
          ...
          util::wutil::calculate_difference(kkbase_x[l] + 13, kkbase_y[l] + 10,
                               ((player_x[l2] + ((player_x_speed[l2] >> 8) *
                                                 (tdistance << 8) / 1700)) >>
                                8),
                               ((player_y[l2] + ((player_y_speed[l2] >> 8) *
                                                 (tdistance << 8) / 1700)) >>
                                8),
                               &distance, &angle);
...
void ai_evade_terrain(int number) {
  ...
  x_kohta = ((((player_x_speed[number] * AVOID_TERRAIN_FRAMES) >> 8) +
              player_x[number]) >>
             8);
  y_kohta = ((player_y[number] -
              ((player_y_speed[number] * AVOID_TERRAIN_FRAMES) >> 8)) >>
             8) +
            PLANE_MODIFICATION * 2;
  if (x_kohta >= 2400)
    x_kohta = 2399;
  if (y_kohta >= 200)
    y_kohta = 199;
  ...
  if ((number % 4 == 1 || number % 4 == 2) && config.current_multilevel == 5 &&
      ...
      player_y[number] > 43000 && going_up > 0) {
...
void do_ai(int number) {
  ...
  for (flaggy = 0; flaggy < 16; flaggy++) {
    ...
    util::wutil::calculate_difference(
        ...
        &distance_enemy[flaggy], &angle_enemy[flaggy]);

    angle_difference[flaggy] =
        abs((player_angle[number] >> 8) - angle_enemy[flaggy]);
  ...
  case AIM_LAND:
    ...
    case 0:
      if ((player_upsidedown[number] && (!going_left)) ||
      ...
      if (going_left) {
        xdistance =
            -((leveldata.airfield_x[player_tsides[number]] +
               (leveldata.airfield_lenght[player_tsides[number]] >> 1)) -
              player_x_8[number]);

      } else {
        xdistance = ((leveldata.airfield_x[player_tsides[number]] +
                      (leveldata.airfield_lenght[player_tsides[number]] >> 1)) -
                     player_x_8[number]);
      }

      ydistance =
          leveldata.airfield_y[player_tsides[number]] - player_y_8[number];
      ...
      if (going_left) {
        ...
        if ((player_angle[number] <= ((360 - HIT_ANGLE) << 8)) &&
            player_angle[number] > 50000) {
          controls_up[number] = 1;
          controls_down[number] = 0;
        }
      }
      ...
      if (ydistance > (xdistance / 3)) {
        if (going_left) {
          ...
          if ((player_angle[number] >
               ((360 - HIT_ANGLE) << 8) + initial_turn) ||
              player_angle[number] < 23041) {
            controls_up[number] = 0;
            controls_down[number] = 1;
          }
        }
        ...
        if (going_left) {
          ...
          if (player_angle[number] < ((TAIL_HIT_ANGLE) << 8) ||
              player_angle[number] > 69119) {
            controls_up[number] = 1;
            controls_down[number] = 0;
          }
        }
  ...
  case AIM_IMMELMAN:
    ...
    case 1:
      ...
      if (going_left && (player_angle[number] < (190 * 256)))
        controls_down[number] = 1;
      else if ((!going_left) && (player_angle[number] > (350 * 256) ||
                                 player_angle[number] < (90 * 256)))
        controls_down[number] = 1;
      break;
  ...
  case AIM_CHASE_PLANE:
    if ((!player_ammo[number]) ||
        distance_enemy[mission_target[number]] > AIM_GIVEUPFIGHT_DISTANCE ||
        !plane_present[mission_target[number]]) {
      current_mission[number] = AIM_NOMISSION;
      ...
      trueangle =
          (player_angle[number] >> 8) - angle_enemy[mission_target[number]];
    ...
    if (controls_up[number] && player_speed[number] < (1800)) {
      ...
      if (going_up &&
          (player_angle[number] > 32000 || player_angle[number] < 14080)) {
        ai_turn_down(number);
      }

    } else if (controls_up[number] &&
               player_speed[number] < (2350 + (player_bombs[number] << 6))) {
  ...
  case AIM_EVADE_TERRAIN:
    ...
    if ((player_x[number] < (80 + (player_speed[number] >> 5)) * 256 &&
         going_left) ||
        (player_x[number] > (2320 - (player_speed[number] >> 5)) * 256 &&
         !going_left)) {
      if ((wide_terrain_level[player_x_8[number]] - (player_y_8[number])) >
          ((player_y_8[number]) + 15))
        current_mission[number] = AIM_SPLITS;
  ...
  if ((player_on_airfield[number] - 1) == player_tsides[number] &&
      !player_speed[number] &&
      (!player_gas[number] || current_mission[number] == AIM_LAND)) {
    plane_wants_in[number] = 1;
  }

  if ((!player_bombs[number] && !player_ammo[number]) ||
      player_gas[number] < 500 ||
    ...
    if ((xdistance < 600) && (xdistance > 100) &&
        (ydistance + 5 > (xdistance / 3)) &&
        (ydistance - 5 < (xdistance / 3))) {

      if (player_speed[number] < 2000 || (ydistance > 130) ||
          (ydistance - (player_speed[number] >> 6)) > 0) {
  ...
  if (player_bombs[number] && !in_closing[number]) {
    ...
    if (((player_angle[number] <= 23040 || player_angle[number] >= 69120) &&
         !player_upsidedown[number]) ||
        ((player_angle[number] >= 23040 && player_angle[number] <= 69120) &&
         player_upsidedown[number]))
  ...
  if (plane_present[number] && player_ammo[number] &&
      (player_last_shot[number] > SHOTS_RATE)) {
    for (flaggy = 0; flaggy < 16; flaggy++) {
      if (flaggy == number)
        continue;
      ...
      if (distance_enemy[flaggy] < (SHOT_AT_RANGE - 20) &&
          angle_difference[flaggy] < 8) {
        return;
      }
    }
    ...
    for (flaggy = 0; flaggy < 16; flaggy++) {
      ...
      if (distance_enemy[flaggy] < SHOT_AT_RANGE &&
          angle_difference[flaggy] < SHOT_AT_ANGLE) {
