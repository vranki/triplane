...
void do_shots() {
  ...
  for (l = 0; l < MAX_SHOTS; l++) {
    if (shots_flying_x[l]) {
      ...
      if ((shots_flying_x[l] < 0) ||
          ((shots_flying_x[l] >> 8) >= NUMBER_OF_SCENES * 160) ||
          shots_flying_y[l] < 0 || (shots_flying_y[l] >> 8) >= 200)
        shots_flying_x[l] = 0;
      else {
        kohta = level_bitmap[(shots_flying_x[l] >> 8) +
                             (shots_flying_y[l] >> 8) * 2400];
      ...
      for (l2 = 0; l2 < 16; l2++) {
        if (plane_present[l2])
          if (((player_x[l2] + 2304) > shots_flying_x[l]) &&
              ...
              ((player_y[l2] + 2304) > shots_flying_y[l]) &&
              ((player_y[l2] - 2304) < shots_flying_y[l]))
            if (plane_p[l2][(player_angle[l2] >> 8) / 6][player_rolling[l2]]
                       [player_upsidedown[l2]]
                       [(shots_flying_x[l] >> 8) - (player_x_8[l2]) + 10 +
                        ((shots_flying_y[l] >> 8) - (player_y_8[l2]) + 10) *
                            20] != 255) {
              ...
              if (player_endurance[l2] < 1) {
                if (!player_spinning[l2] && !in_closing[l2]) {
                  if (shots_flying_owner[l] != -1) {
                    if (player_sides[l2] !=
                        player_sides[shots_flying_owner[l]]) {
...
void start_shot(int player) {
  ...
  if (config.gunshot_sounds && config.sound_on && config.sfx_on) {
    if (plane_type == 0 || plane_type > 3)
      plane_type = 1;
    play_2d_sample(sample_konsu[plane_type], player_x_8[solo_country],
                   player_x_8[player]);
  }
}
...
void do_fobjects() {
  ...
  for (l = 0; l < MAX_FLYING_OBJECTS; l++) {
    if (fobjects[l].x) {
      ...
      if ((fobjects[l].y < 0) || (fobjects[l].x < 0) ||
          ((fobjects[l].x >> 8) >= NUMBER_OF_SCENES * 160) ||
          (fobjects[l].y >> 8) >= 200) {
        fobjects[l].x = 0;
        continue;
      }

      if ((level_bitmap[(fobjects[l].x >> 8) + (fobjects[l].y >> 8) * 2400]) <
              112 ||
          ((level_bitmap[(fobjects[l].x >> 8) + (fobjects[l].y >> 8) * 2400])) >
              119) {
        fobjects[l].x = 0;
        continue;
      }
        ...
        for (l2 = 0; l2 < 16; l2++) {
          if (plane_present[l2]) {
            if (((player_x[l2] + 2304) > fobjects[l].x) &&
                ((player_x[l2] - 2304) < fobjects[l].x) &&
                ((player_y[l2] + 2304) > fobjects[l].y) &&
                ((player_y[l2] - 2304) < fobjects[l].y))
              if (plane_p[l2][(player_angle[l2] >> 8) / 6][player_rolling[l2]]
                         [player_upsidedown[l2]]
                         [(fobjects[l].x >> 8) - (player_x_8[l2]) + 10 +
...
void do_bombs() {
  ...
  for (l = 0; l < MAX_BOMBS; l++) {
    ...
    if (bomb_y[l] >= 0)
      if ((level_bitmap[(bomb_x[l] >> 8) + (bomb_y[l] >> 8) * 2400]) < 112 ||
          ((level_bitmap[(bomb_x[l] >> 8) + (bomb_y[l] >> 8) * 2400])) > 119 ||
          ((bomb_y[l] >> 8) >= 200)) {
        start_bomb_explo(l);
    ...
    if (bomb_age[l]++ >
        BOMB_PRIME_TIME) { // Dont' destroy players if not primed yet
      for (l2 = 0; l2 < 16; l2++) {
        if (plane_present[l2]) {
          if (((player_x[l2] + 2304) > bomb_x[l]) &&
              ((player_x[l2] - 2304) < bomb_x[l]) &&
              ((player_y[l2] + 2304) > bomb_y[l]) &&
              ((player_y[l2] - 2304) < bomb_y[l]))
            if (plane_p[l2][(player_angle[l2] >> 8) / 6][player_rolling[l2]]
                       [player_upsidedown[l2]]
                       [(bomb_x[l] >> 8) - (player_x_8[l2]) + 10 +
                        ((bomb_y[l] >> 8) - (player_y_8[l2]) + 10) * 20] !=
                255) {
              bomb_x[l] = 0;
...
void start_bomb_explo(int bb, int hitted) {
  ...
  for (l = 0; l < MAX_AA_GUNS; l++)
    if (kkbase_x[l] && kkbase_status[l] != 2) {
      if ((bomb_x[bb] >> 8) >= kkbase_x[l] &&
          (bomb_x[bb] >> 8) <= (kkbase_x[l] + 25) &&
          (bomb_y[bb] >> 8) >= (kkbase_y[l] - 5) &&
          (bomb_y[bb] >> 8) <= (kkbase_y[l] + 30)) {
  ...
  for (l = 0; l < MAX_STRUCTURES; l++)
    if ((structures[l][1] != nullptr) && (!struct_state[l])) {
      if ((bomb_x[bb] >> 8) >= leveldata.struct_x[l] &&
          (bomb_x[bb] >> 8) <= (leveldata.struct_x[l] + struct_width[l]) &&
          (bomb_y[bb] >> 8) >= (leveldata.struct_y[l] - 5) &&
          (bomb_y[bb] >> 8) <= (leveldata.struct_y[l] + 5 + struct_heigth[l])) {
        if (config.flames)
          start_flame(leveldata.struct_x[l],
                      leveldata.struct_y[l] + struct_heigth[l],
                      struct_width[l]);
        ...
        if (player_sides[leveldata.struct_owner[l]] ==
            player_sides[bomb_owner[bb]])
          hitted = -1;
        ...
        if (leveldata.struct_hit[l]) {
          structures[l][1]->blit_to_bitmap(maisema, leveldata.struct_x[l],
                                           leveldata.struct_y[l]);
        }
      }
    }
  ...
  for (l = 0; l < MAX_INFANTRY; l++) {
    if (infan_x[l] && infan_state[l] < 3) {
      if ((bomb_x[bb] >> 8) >= (infan_x[l] - 30) &&
          (bomb_x[bb] >> 8) <= (infan_x[l] + 44) &&
          (bomb_y[bb] >> 8) >= (infan_y[l] - 20) &&
          (bomb_y[bb] >> 8) <= (infan_y[l] + 30)) {
